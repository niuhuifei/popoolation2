...
sub get_basic_mpileupparser
{
    ...
    my $minqual=shift;
    my $encoder=[];
    foreach my $qe (@$quality_encoding)
    {
        $qe=lc($qe);
        die "Encoder $quality_encoding not supported" unless exists($qualhash->{$qe});
        push @$encoder, $qualhash->{$qe};
    }

    
    my $subparser=_get_subnucsqualparser($minqual);
    return sub
    {
        my $line=shift;
        ...
        while(@ar)
        {
            ...
            my $entry=$subparser->($cov,$nucs,$qual,$rc,$encoder->[$counter]);
            push @$entries,$entry;
            $counter++;
        }
